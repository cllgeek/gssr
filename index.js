  ...
  connListener = (req, res) => {
      ...
      _header = !cors
        ...
        : {
            ...
            "Access-Control-Allow-Headers":
              "Content-Type, Authorization, Content-Length, X-Requeseted-With, Accept, x-csrf-token, origin"
          };
    ...
    if (pxval) {
      ...
      req.addListener("end", () => {
        ...
        res.write(
          iconv.encode(JSON.stringify(pxval), "utf-8").toString("binary"),
          "binary"
        );
        res.end();
    ...
    if (
      ...
      fs.existsSync(filename + "/index.html")
    )
      filename += "index.html";
...
const commandLog = (status, req, res) => {
  ...
  if (code === 200) {
    console.log(
      chalk.bold.green("INFO " + req.method + " " + code.toString()) + " " + req.url
    );
  } else {
    console.log(
      chalk.bold.green("INFO " + req.method + " " + code.toString()) + "" + req.url
...
const serverS = argv => {
  ...
  if (argv && argv.port === true)
    pt = __port(argv && argv.cors) ? (cors = true) : (cors = false);
